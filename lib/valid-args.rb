...
class ValidArgs < ActiveRecord::Base
  def self.is_args_larger_than_number?(args, number)
    return true if args.length > number
    return false
  end

  def self.is_args_smaller_than_number?(args, number)
    return true if args.length < number
    return false
  end

  def self.is_args_equal_number?(args, number)
    return true if args.length == number
    return false
  end

  def self.is_exist_mode?(mode)
    return true if Mode.find_by(play_mode: mode)
    return false
  end

  def self.is_exist_player?(players_name)
